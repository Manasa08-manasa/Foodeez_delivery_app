...
class _IncomingRequestAlertState extends ConsumerState<IncomingRequestAlert> with SingleTickerProviderStateMixin {
      ...
      child: Container(
        ...
        child: SafeArea(
          ...
          child: ConstrainedBox(
            constraints: BoxConstraints(maxHeight: maxSheetHeight, maxWidth: Responsive.contentWidth(context)),
            child: Container(
              width: double.infinity,
              padding: Responsive.screenPadding(context, horizontal: 20, vertical: 22).copyWith(bottom: 24),
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('New delivery request', style: AppText.display(size: Responsive.fontSize(context, 20))),
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          Expanded(child: _stat('+₹${req.payout}', 'payout', AppColors.green)),
                          _divider(),
                          Expanded(child: _stat('${req.totalKm}', 'km total', AppColors.ink)),
                          _divider(),
                          Expanded(child: _stat('${req.totalMins}', 'min', AppColors.ink)),
                        ],
                      ),
                    ),
                    Container(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text('${req.restaurantName} · ${req.restaurantAddress.split(',').first}', maxLines: 2, overflow: TextOverflow.ellipsis, style: AppText.body(size: 12.5, weight: FontWeight.w700)),
                                Padding(padding: const EdgeInsets.only(top: 12), child: Text('${req.customerAddress} · ${req.totalKm} km', maxLines: 2, overflow: TextOverflow.ellipsis, style: AppText.body(size: 12.5, weight: FontWeight.w700))),
                              ],
                            ),
                          ),
                        ],
                      ...
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                cod ? 'Collect ₹${req.orderTotal} in cash' : '✓ Prepaid · ${paymentMethodLabel(req.paymentMethod)}',
                                maxLines: 2,
                                overflow: TextOverflow.ellipsis,
                                style: AppText.body(size: 11, weight: FontWeight.w800, color: cod ? AppColors.goldDeep : AppColors.green),
                              ),
                            ),
                          ],
                        ),
  ...
  Widget _stat(String value, String label, Color color) => Column(
        children: [
          FittedBox(fit: BoxFit.scaleDown, child: Text(value, style: AppText.display(size: 22, color: color))),
          Text(label, style: AppText.body(size: 10.5, weight: FontWeight.w600, color: AppColors.bodyGrey)),
        ],
      );

  Widget _divider() => Container(width: 1, height: 34, margin: const EdgeInsets.symmetric(horizontal: 8), color: AppColors.dividerBorder2);
}
